...
const SummaryPanel = (p: PanelProps) => {
  ...
  return (
    <div style={{
      ...
      {!bootstrapPage && !settingsPage && !generatePage && <>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <WaveIcon size={16} color={C.blue} />
          <span style={{ fontSize: 13, fontWeight: 800, letterSpacing: -0.2, color: C.text }}>Drift</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          {/* Generate page shell */}
          <button
            onClick={() => { setGenOutput(null); setGenError(null); setGeneratePage(true) }}
            title="Generate a page shell using your design system"
            style={{
              width: 28, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'center',
              background: C.btnBg, border: `1px solid ${C.panelBorder}`,
              borderRadius: 8, cursor: 'pointer', color: C.textSub,
            }}>
            {/* Layout scaffold icon */}
      ...
      <div style={{
        ...
        <button
          ...
          title="Quick scan — top-level components only, stops at DS component boundaries"
          style={{
            padding: '3px 10px', borderRadius: 20, border: 'none', cursor: 'pointer',
            fontSize: 11, fontWeight: 600, fontFamily: 'Inter, sans-serif',
            background: p.surfaceMode ? C.muted : 'transparent',
            color: p.surfaceMode ? '#fff' : C.muted,
          }}>Quick</button>
        <button
          onClick={p.onToggleSurface}
          title="Full scan — descends inside DS components to find internal helpers and nested custom elements"
          style={{
            padding: '3px 10px', borderRadius: 20, border: 'none', cursor: 'pointer',
            fontSize: 11, fontWeight: 600, fontFamily: 'Inter, sans-serif',
            background: !p.surfaceMode ? C.muted : 'transparent',
