/**
 * injected.js — executes in the PAGE context (not isolated content script context).
 * This gives it access to window.__reactFiber$ and React internals.
 * Loaded via a <script src> tag injected by content.js.
 */

// ─── Fiber scanner (identical logic to scanner.js, inlined here) ──────────────
...
;(function() {
  ...
  function runScan(dsComponentsList) {
    const dsComponents = new Set(dsComponentsList)
    const rootFiber = findRootFiber()
    if (!rootFiber) {
      return { error: 'React not found on this page', pct: 0, total: 0, ds: 0, gaps: [], tokenViolations: [] }
    }
  ...
  window.addEventListener('__dd_scan_request__', (e) => {
    const { requestId, components } = e.detail
    const result = runScan(components)
    window.dispatchEvent(new CustomEvent('__dd_scan_result__', { detail: { requestId, result } }))
  })
})()
