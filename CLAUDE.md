...
| `Modal` | `src/stories/Modal.tsx` | Dialog overlay with header, body, footer slots |
| `Toast` | `src/stories/Toast.tsx` | Transient notification: success/error/warning/info |
| `Dropdown` | `src/stories/Dropdown.tsx` | Select/menu dropdown with option list |
...
npm run drift-check    # CI drift check (exit 1 if coverage < threshold)
